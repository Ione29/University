...
def intersect(l1: Line2D, l2: Line2D)(start: Int, end: Int): Boolean = {
    
    def loop(pos: Int): Boolean = {
        if(l1(pos) == l2(pos)) true
        else if(pos > end) false
        else loop(pos + 1)
    }
...
val line1: Line2D = x => x + 1
val line2: Line2D = x => x + 1

intersect(line1, line2)(0, 10)
...
def larger(l1: Line2D, l2: Line2D)(start: Int, end: Int): Boolean = {
    
    def loop(pos: Int): Boolean = {
        if(l1(pos) <= l2(pos)) false
        else if (pos > end) true
        else loop(pos + 1)
    }
...
val line3: Line2D = x => x + 2
val line4: Line2D = x => x + 1

larger(line3, line4)(0, 10)